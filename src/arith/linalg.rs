...
impl<F: PrimeField> Matrix<F> {
    ...
    pub fn par_mat_vec_mul(&self, rhs: &Vec<F>) -> Vec<F> {
        assert!(
            self.width == rhs.len(),
            "Matrix width must match vector length"
        );
...
impl<const D: usize, F: PrimeField> Matrix<CyclotomicRing<D, F>> {
    ...
    pub fn mle_evals_num_vars(&self) -> usize {
        (self.data.len() * D).next_power_of_two().ilog2() as usize
    }
    ...
    pub fn to_mle_evals(&self) -> (Vec<F>, usize) {
        ...
        let num_variables = self.mle_evals_num_vars();
